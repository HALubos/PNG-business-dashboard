...
function buildHref(projekt: string, obdobi: string): string {
  ...
  p.set("obdobi", obdobi);
  return `/marketing/web-analytika?${p.toString()}`;
}
        ...
        {canExport ? (
          <>
            <Button asChild variant="outline" size="sm">
              <a href={`/api/mkt-analytics/export?format=xlsx${exportQuery}`}>
                <Download className="size-4" /> XLSX
              </a>
            </Button>
            <Button asChild variant="outline" size="sm">
              <a href={`/api/mkt-analytics/export?format=csv${exportQuery}`}>
